...
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE policyname = 'affirmations_user_isolation') THEN
        CREATE POLICY affirmations_user_isolation ON affirmations
        FOR ALL USING (user_id = current_setting('app.current_user_id')::UUID);
    END IF;
    
    IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE policyname = 'books_user_isolation') THEN
        CREATE POLICY books_user_isolation ON books
        FOR ALL USING (user_id = current_setting('app.current_user_id')::UUID);
    END IF;
    
    IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE policyname = 'reading_sessions_user_isolation') THEN
    ...
    IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE policyname = 'tasks_user_isolation') THEN
        CREATE POLICY tasks_user_isolation ON tasks
        FOR ALL USING (user_id = current_setting('app.current_user_id')::UUID);
    END IF;
    
    IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE policyname = 'learning_items_user_isolation') THEN
        CREATE POLICY learning_items_user_isolation ON learning_items
        FOR ALL USING (user_id = current_setting('app.current_user_id')::UUID);
    END IF;
    
    IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE policyname = 'job_applications_user_isolation') THEN
        CREATE POLICY job_applications_user_isolation ON job_applications
        FOR ALL USING (user_id = current_setting('app.current_user_id')::UUID);
    END IF;
    
    IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE policyname = 'user_preferences_user_isolation') THEN
...
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_trigger WHERE tgname = 'update_affirmations_updated_at') THEN
    ...
    IF NOT EXISTS (SELECT 1 FROM pg_trigger WHERE tgname = 'update_learning_items_updated_at') THEN
