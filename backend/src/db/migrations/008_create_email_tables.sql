...
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE policyname = 'contacts_user_isolation') THEN
        CREATE POLICY contacts_user_isolation ON contacts
        FOR ALL USING (user_id = current_setting('app.current_user_id')::UUID);
    END IF;
    
    IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE policyname = 'email_logs_user_isolation') THEN
        CREATE POLICY email_logs_user_isolation ON email_logs
        FOR ALL USING (user_id = current_setting('app.current_user_id')::UUID);
    END IF;
    
    IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE policyname = 'email_templates_user_isolation') THEN
